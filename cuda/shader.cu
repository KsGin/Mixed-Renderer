...
/*************************************************************Pixel Shader*************************************************************************************************/
__device__ Color& CubePixelShader(Pixel& pixel , Args args) {
	//
...
__device__ Color& WaterPixelShader(Pixel& pixel , Args args) {
	//
...
__device__ Color& LightPixelShader(Pixel& pixel , Args args) {
	//
	// auto uv = pixel.uv / 2 + Math::Vector2(args.bis + 0.25 , 0);
	//
	// CLAMP01(uv._x);
...
/***********************************************************Shader 调用******************************************************************************************************/
__global__ void KernelPixelShader(Color* colors, Pixel* pixels, 
                                  Args cubeArgs , Args waterArgs , const int numElements ) {
	const int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if (idx < numElements) {
		switch (pixels[idx].sType) {
			case CUBE : colors[idx] = CubePixelShader(pixels[idx] , cubeArgs); break;
			case WATER : colors[idx] = WaterPixelShader(pixels[idx] , waterArgs); break;
...
extern "C" void CallPixelShader(std::unordered_map<ShaderType , ShaderData>& sDataMaps , const std::vector<Pixel>& pixels, std::vector<Color>& colors) {
	...
	//
	// 64
	KernelPixelShader<<<(numPixels + 63) / 64 , 64>>>(dColors, dPixels , sDataMaps[CUBE].args , sDataMaps[WATER].args , numPixels);
