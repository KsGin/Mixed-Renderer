...
namespace Math {
	...
	public:
		 ...
		 * 重载 +
		 */
		Vector2 operator+(const Vector2& v2) const { return Vector2(_x + v2._x, _y + v2._y); }
		 ...
		 * 重载 *
		 */
		Vector2 operator*(const float val) const { return Vector2(_x * val, _y * val); }

		/**
		 * 判断相等
		 */
		bool operator==(const Vector2& v2) const { return _x == v2._x && _y == v2._y; }
	...
	public:
		 ...
		 * 重载 +
		 */
		Vector3 operator+(const Vector3& v3) const { return Vector3(_x + v3._x, _y + v3._y, _z + v3._z); }
		 ...
		 * 重载 *
		 */
		Vector3 operator*(const float val) const { return Vector3(_x * val, _y * val, _z * val); }

		/**
		 * 重载 ==
		 */
		bool operator==(const Vector3& v3) const { return _x == v3._x && _y == v3._y && _z == v3._z; }
		 ...
		 * 求点积
		 */
		static float dot(const Vector3& vl, const Vector3& vr) { return vl._x * vr._x + vl._y * vr._y + vl._z * vr._z; }
